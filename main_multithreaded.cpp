...
// Function to write tree contents to a file (in-order traversal)
void write_tree(const std::shared_ptr<BpTree<uint64_t, 64>::Node>& node, std::ofstream& outfile) {
    if (node->is_leaf_) {
        ...
        for (int i = 0; i < leaf->count_; ++i) {
            ...
            // Write vector elements with fixed precision
...
int main() {
    ...
    // Timing the multithreaded search phase
    ...
    auto mt_duration = std::chrono::duration_cast<std::chrono::milliseconds>(mt_end - mt_start).count();

    std::cout << "Multithreaded search completed in " << mt_duration << " ms.\n";
    // Now 'results' contains the vectors for found queries, and 'found' indicates which were found
    //for (size_t i = 0; i < queries.size(); ++i) {
        //if (found[i]) {
